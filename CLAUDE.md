...
⚠️ **`paths`-фильтра у `deploy.yml` нет: любой мерж в main передеплоит бота — даже правка
только этого файла.** На пересменке ревизий старая дренируется, пока новая уже поллит → 20–40 с
конфликтов 409 в логах, потом `Connection established`. Само по себе не страшно (aiogram
переподключается), но: 409 сразу после мерджа — это **ожидаемая пересменка**, а не второй поллер;
паниковать надо, только если конфликты идут дальше нескольких минут. Проверка — `grep -i conflict`
по логам с отметкой времени против `createdTime` живой ревизии.
