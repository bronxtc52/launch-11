...
def tool_defs(version: str) -> list[dict]:
    ...
    return [
        ...
        },
        {
            ...
            "description": (
                "Назови продукт коротко (2-6 слов), как только идея ясна — например "
                "«AI-рекрутер для hh.kz». От этого зависит имя файла spec.md, который "
                "получит человек. Вызови ОДИН раз на первом шаге. НЕ используй реплики "
                "человека вроде «да» или «не понял» — это не название."
            ),
            "input_schema": {
                "type": "object",
                "properties": {
                    "name": {"type": "string", "description": "Короткое название продукта."},
                },
                "required": ["name"],
            },
        },
            ...
            "input_schema": {
                ...
                "properties": {
                    ...
                    "options": {"type": "array", "items": {"type": "string"},
                                "description": "ОБЯЗАТЕЛЬНО, если предлагаешь выбор: короткие метки "
                                               "вариантов для засчитывания ответа кодом (например "
                                               "['скорость','регулярность','результат']). ЭТО НЕ "
                                               "замена формулировкам: сами варианты покажи человеку "
                                               "ЦЕЛИКОМ, полными предложениями, в preamble. Метки "
                                               "нужны только чтобы код распознал выбор — он засчитает "
                                               "его сам, не оценивай выбор варианта как неполный."},
                },
                "required": ["question"],
            },
        },
