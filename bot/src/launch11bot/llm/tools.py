...
def tool_defs(version: str) -> list[dict]:
    ...
    return [
        ...
        },
        {
            "name": "assess_answer",
            "description": (
                "Оцени последнюю реплику пользователя относительно заданного вопроса. "
                "Вызывай ПЕРВЫМ, пока есть открытый вопрос. 'answer' — ответил по существу; "
                "'partial' — ответил частично (укажи missing, потом задай уточняющий вопрос); "
                "'offtopic' — это не ответ на вопрос (бот сам повторит вопрос)."
            ),
            "input_schema": {
                "type": "object",
                "properties": {
