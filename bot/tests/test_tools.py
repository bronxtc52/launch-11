...
def test_tool_defs_step_enum_and_lite_only_version():
    ...
    assert defs["set_version"]["input_schema"]["properties"]["version"]["enum"] == ["lite"]
...
async def test_set_version_lite_ok_full_rejected(orch):
    s = await orch.start(1)
    ok = await dispatch(orch, s, "set_version", {"version": "lite"})
    assert ok.ok is True
    bad = await dispatch(orch, s, "set_version", {"version": "full"})
    assert bad.ok is False
    assert bad.session.current_step == "L1"
